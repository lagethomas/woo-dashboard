...
if (!function_exists('lv_settings_screen')) {
    ...
    add_action('admin_enqueue_scripts', function ($hook) {
        // Garante que o CSS seja carregado apenas na página correta
        if ($hook !== 'loja-virtual_page_minha-loja-configuracoes') {
             return;
        }
        
        $css_file = 'assets/css/lojavirtual-settings.css';
        if (file_exists(LV_DIR . $css_file)) {
            wp_enqueue_style('lv-settings-css', LV_URL . $css_file, array(), CODE_VERSION);
        }
    });
    ...
    add_action('admin_init', function () {
        register_setting('lv_dash_group', 'lv_dash_settings', array(
            ...
            'sanitize_callback' => function ($in) {
                if (!isset($_POST['lv_settings_nonce_field']) || !wp_verify_nonce($_POST['lv_settings_nonce_field'], 'lv_settings_nonce')) {
                    add_settings_error('lv_dash_settings', 'lv_nonce_error', 'Erro de segurança. As alterações não foram salvas.', 'error');
        ...
        add_settings_section('lv_sec_main', 'Configurações Gerais', function () {
            echo '<p style="max-width:720px">Ajustes visuais e de conteúdo do painel. As cores afetam bordas, botões, títulos, ícones e barras do gráfico.</p>';
        }, 'lv_dash_settings_page');
        add_settings_field('lv_title', 'Título do Dashboard', function () {
            $o = lv_get_options();
            echo '<input type="text" name="lv_dash_settings[title]" value="' . esc_attr($o['title']) . '" class="regular-text" /><p class="description">Texto exibido no topo do painel.</p>';
        }, 'lv_dash_settings_page', 'lv_sec_main');
        add_settings_field('lv_monthly_goal', 'Meta de Faturamento Mensal', function () {
            $o = lv_get_options();
            ...
            echo '<p class="description">Defina um objetivo de vendas para o mês atual. Deixe em 0 para desativar.</p>';
        }, 'lv_dash_settings_page', 'lv_sec_main');
        add_settings_field('lv_dark_mode', 'Aparência', function () {
            $o = lv_get_options();
            $checked = !empty($o['dark_mode']) ? 'checked' : '';
            echo '<label><input type="checkbox" name="lv_dash_settings[dark_mode]" value="1" ' . $checked . '> Ativar modo escuro (Dark Mode)</label>';
        }, 'lv_dash_settings_page', 'lv_sec_main');
        add_settings_field('lv_font_scale', 'Fonte e Tamanho base', function () {
            $o = lv_get_options();
            ...
            echo '<div style="display:flex;gap:16px;align-items:center;flex-wrap:wrap"><label>Fonte:&nbsp;<select name="lv_dash_settings[font]">';
            foreach ($fonts as $k => $label) {
                echo '<option value="' . esc_attr($k) . '" ' . selected($o['font'], $k, false) . '>' . esc_html($label) . '</option>';
            }
            echo '</select></label><label>Tamanho base:&nbsp;<select name="lv_dash_settings[scale]">';
            foreach (array('90%','100%','110%','120%','130%') as $opt) {
                echo '<option value="' . esc_attr($opt) . '" ' . selected($o['scale'], $opt, false) . '>' . esc_html($opt) . '</option>';
            }
            echo '</select></label></div>';
        }, 'lv_dash_settings_page', 'lv_sec_main');
        add_settings_field('lv_colors', 'Cores', function () {
            $o = lv_get_options();
            $c=$o['colors'];
            ...
            $row('text','Cor de texto');
            $row('link','Cor de link');
            $row('title','Cor do título');
            $row('button','Cor do botão');
            $row('button_text','Cor do texto do botão');
            $row('border','Cor da borda');
            $row('bars','Cor das barras do gráfico');
            $row('icons','Cor dos ícones');
        }, 'lv_dash_settings_page', 'lv_sec_main');

        add_settings_section('lv_sec_content', 'Conteúdo do Dashboard', function () {
            echo '<p style="max-width:720px">Controle quais elementos são exibidos e a ordem dos cards de métricas.</p>';
        }, 'lv_dash_settings_page');
        add_settings_field('lv_cards', 'Cards e Boxes Exibidos', function () {
            $o = lv_get_options();
            $c=$o['cards'];
            ...
            foreach ($items as $k => $label) {
                ...
                echo '<label><input type="checkbox" name="lv_dash_settings[cards][' . esc_attr($k) . ']" value="1" ' . $checked . '> ' . esc_html($label) . '</label>';
            }
            echo '</fieldset>';
        }, 'lv_dash_settings_page', 'lv_sec_content');
        add_settings_field('lv_order', 'Ordem dos Cards de Métricas', function () {
            $o = lv_get_options();
            ...
            foreach ($labels as $key => $label) {
                ...
                echo '<label style="display:flex;align-items:center;gap:8px"><span style="min-width:180px">' . esc_html($label) . ':</span><select name="lv_dash_settings[order][' . esc_attr($key) . ']">';
        ...
        add_settings_field('lv_period_status', 'Configurações de Métricas', function () {
            $o = lv_get_options();
            $sel = $o['statuses_for_metrics'];
            echo '<div style="display:flex;gap:24px;flex-wrap:wrap;align-items:center;"><label>Meses no gráfico:&nbsp;<input type="number" min="3" max="24" name="lv_dash_settings[months]" value="' . esc_attr(intval($o['months'])) . '" class="small-text" /></label><label>Visão padrão do gráfico:&nbsp;<select name="lv_dash_settings[chart_default_view]"><option value="value" ' . selected($o['chart_default_view'], 'value', false) . '>Valor (R$)</option><option value="orders" ' . selected($o['chart_default_view'], 'orders', false) . '>Pedidos</option></select></label></div><hr style="margin: 16px 0; border: 0; border-top: 1px solid #ddd;"><div><div style="margin-bottom:6px;font-weight:600">Status considerados nas métricas</div>';
            ...
            foreach ($all as $key => $label) {
                ...
                echo '<label style="display:inline-block;margin:4px 12px 4px 0;"><input type="checkbox" name="lv_dash_settings[statuses_for_metrics][]" value="' . esc_attr($key) . '" ' . $checked . '> ' . esc_html($label) . '</label>';
            }
            echo '</div>';
        }, 'lv_dash_settings_page', 'lv_sec_content');
    });
    ...
    function lv_settings_screen()
    {
        ...
        echo '<div class="wrap"><h1 style="margin-bottom:12px;">' . esc_html__('Configurações do Dashboard', 'lojavirtual-dashboard') . ' <span class="lv-version-badge">v' . esc_html('3.0.0') . '</span></h1>';
        ...
        echo '<a href="' . esc_url($reset_url) . '" class="button button-secondary">Restaurar padrão</a>';
        echo '</div></form>';
        echo '<div class="lv-footer-brand" style="margin:16px 0 0;text-align:right;color:#6b7a8c;">Desenvolvido por <a href="https://wpmasters.com.br" target="_blank" rel="noopener"><strong style="color:#5b6470;">Thomas Marcelino</strong></a></div></div>';
