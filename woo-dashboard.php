...
// Versionamento para evitar cache de CSS/JS
define('CODE_VERSION', time());
...
add_action('admin_enqueue_scripts', function ($hook) {
    ...
    if (file_exists(LV_DIR . $css_path)) {
        wp_enqueue_style('lv-dashboard-css', LV_URL . $css_path, array(), CODE_VERSION);
    }
...
/* ===== Funções Auxiliares com Cache (Transients API) ===== */
...
function lv_handle_csv_export() { if (isset($_GET['lv_export_csv']) && $_GET['lv_export_csv'] == 'true' && current_user_can('manage_options')) { header('Content-Type: text/csv; charset=utf-8'); header('Content-Disposition: attachment; filename=ultimas-vendas-' . date('Y-m-d') . '.csv'); $output = fopen('php://output', 'w'); fputcsv($output, array('Pedido ID', 'Cliente', 'Email Cliente', 'Total (R$)', 'Status', 'Data', 'Itens')); $o = lv_get_options(); $orders = lv_last_orders(100, $o['statuses_for_metrics']); if (!empty($orders)) { foreach ($orders as $order) { $items_list = array(); foreach ($order->get_items() as $item) { $items_list[] = $item->get_name() . ' (x' . $item->get_quantity() . ')'; } $row = array($order->get_id(), $order->get_formatted_billing_full_name(), $order->get_billing_email(), $order->get_total(), wc_get_order_status_name($order->get_status()), $order->get_date_created()->date('d/m/Y H:i'), implode('; ', $items_list)); fputcsv($output, $row); } } fclose($output); exit; } }
...
function lv_dashboard_screen()
{
    ...
    echo '<h1 class="lv-title-admin" style="color:var(--lv-title);margin:8px 0 14px;font-weight:700;">' . esc_html($o['title']) . ' <span class="lv-version-badge">v' . esc_html('1.0') . '</span></h1>';
    ...
    if (!empty($o['cards']['chart'])) {
        ...
        (function(){
            ...
            ready(function(){
                ...
                var el = document.getElementById('lvChartMonthly'); if(!el) return;
                var ctx = el.getContext('2d');
                var css = getComputedStyle(document.querySelector('.wrap'));
                ...
                function withAlpha(hex, a){ var c = hex.replace('#',''); if (c.length === 3) c = c.split('').map(function(x){return x+x}).join(''); var r = parseInt(c.substr(0,2),16), g = parseInt(c.substr(2,2),16), b = parseInt(c.substr(4,2),16); return 'rgba('+r+','+g+','+b+','+a+')'; }
                ...
                var chart = new Chart(ctx, {
                    type: 'bar',
                    data: { labels: labels, datasets: [{ label: 'Vendas por Mês', data: salesValues, backgroundColor: withAlpha(baseColor, 0.6), borderColor: baseColor, borderWidth: 1, borderRadius: 6 }] },
                    options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false }, tooltip: { callbacks: { title: function(items){ return labelsFull[items[0].dataIndex] || items[0].label; }, label: function(ctx){ return currencyFormatter.format(ctx.parsed.y); } } } }, scales: { y: { beginAtZero: true, grid: { color: withAlpha(textColor, 0.1) }, ticks: { color: textColor, callback: function(value){ return currencyFormatter.format(value); } } }, x: { grid: { display: false }, ticks: { color: textColor } } } }
                });
                ...
                if (controls) {
                    controls.addEventListener('click', function(e){
                        ...
                        if (view === 'value') { chart.data.datasets[0].data = salesValues; chart.data.datasets[0].label = 'Valor das Vendas'; chart.options.scales.y.ticks.callback = function(value){ return currencyFormatter.format(value); }; chart.options.plugins.tooltip.callbacks.label = function(ctx){ return 'Vendas: ' + currencyFormatter.format(ctx.parsed.y); };
                        } else { chart.data.datasets[0].data = orderCounts; chart.data.datasets[0].label = 'Número de Pedidos'; chart.options.scales.y.ticks.callback = function(value){ if (Number.isInteger(value)) return numberFormatter.format(value); }; chart.options.plugins.tooltip.callbacks.label = function(ctx){ return 'Pedidos: ' + numberFormatter.format(ctx.parsed.y); }; }
                        chart.update();
                    });
                }
    ...
    if (!empty($o['cards']['payment_methods'])) {
        ...
        (function(){
            function ready(fn){ if(document.readyState !== 'loading'){ fn(); } else { document.addEventListener('DOMContentLoaded', fn); } }
            ready(function(){ if (typeof Chart === 'undefined') return; var ctx = document.getElementById('lvPaymentChart')?.getContext('2d'); if (!ctx) return; new Chart(ctx, { type: 'doughnut', data: { labels: <?php echo $payment_labels; ?>, datasets: [{ label: 'Total de Vendas', data: <?php echo $payment_values; ?>, backgroundColor: ['#3b82f6','#10b981','#f97316','#ef4444','#8b5cf6','#14b8a6'], hoverOffset: 4 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'top' } } } }); });
        })();
        </script>
    <?php }
    ...
    if (!empty($o['cards']['top_products'])) {
        ...
        if (!empty($top_products)) {
            foreach ($top_products as $product) {
                echo '<li><a href="' . get_edit_post_link($product->product_id) . '">' . esc_html($product->post_title) . '</a><span>' . esc_html($product->total_quantity) . ' vendidos</span></li>';
            }
        } else {
